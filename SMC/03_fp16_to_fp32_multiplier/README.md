...
- **流水线设计**: 1个时钟周期延迟，支持高频运行

## 文件结构

```
03_fp16_to_fp32_multiplier/
├── README.md                          # 本文件
├── testplan.md                        # 测试计划文档
├── fp16_to_fp32_multiplier.v          # 主设计文件
├── tb_fp16_to_fp32_multiplier.v       # testbench文件
...
1. **数据写入阶段**: 
   - 写入S0-S3寄存器：1个时钟周期
   - 写入S4-S7寄存器：4个时钟周期（分别写入S4,S5,S6,S7）
...
- **寄存器组交叉测试**: 验证不同寄存器组组合
- **无效指令测试**: 验证控制逻辑正确性
- **SoftFloat对比**: 使用Berkeley SoftFloat库作为金标准

### 运行仿真

```bash
cd /home/Sunny/SMC/03_fp16_to_fp32_multiplier
./run_sim.sh
```
